...
class GenereatePotFile extends Command {
	...
	protected function execute(InputInterface $input, OutputInterface $output){
		...
		foreach($total_keys as $filename => $keys){
			...
			$compiled_elements[] = implode(PHP_EOL,[
				'# --------------'.str_pad('',strlen($this->getRelativeFilename($filename)),'-'),
				'# File: '.$this->getRelativeFilename($filename),
	 ...
	 * @return string
	 */
	protected function getRelativeFilename(string $file):string{
		return str_replace(Path::activeThemePath('resources/views').'/','',$file);
	}
	...
	protected function generateFile(array $elements,array $inspected_files){
		...
		/* Map Files */
		$inspected_files = array_map([$this,'getRelativeFilename'],$inspected_files);
		...
		$header = [
			...
			'# Date: '.Carbon::now(),
			$pad,
			'# Inspected files: '.count($inspected_files),
			$pad,
		];
